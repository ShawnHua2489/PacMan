...
class Node:
    def __init__(self, state, action, path_cost, parent = None):
        self.state = state
        self.action = action
...
def depthFirstSearch(problem):
    ...
    while not fringe.isEmpty():
        ...
        for child in problem.getSuccessors(node.state):
            if child[0] not in closed: #not in set
...
def uniformCostSearch(problem):
    ...
    while not fringe.isEmpty():
        ...
        if node.state not in closed:
            closed.add(node.state)
            for child in problem.getSuccessors(node.state):
                fringe.push(Node(child[0], child[1], child[2], node), child[2] + problem.getCostOfActions(node.solution()))
    return []
        ...
        if node.state not in closed:
            ...
            for child in problem.getSuccessors(node.state):
                fringe.push(Node(child[0], child[1], child[2], node), child[2] + \
                            problem.getCostOfActions(node.solution()) + heuristic(child[0], problem))
    return []
